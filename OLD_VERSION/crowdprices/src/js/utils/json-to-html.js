 ...
 */
function ConvertJsonToTable(parsedJson, tableId, headerslist, tableClassName, linkText)
{
...
function ConvertJSONtoSummaryTable(parsedJson, tableId, headerslist, tableClassName, linkText, valuesIndex, removeHeaders)
{
    //Patterns for links and NULL value
    var italic = '<i>{0}</i>';
    var link = linkText ? '<a href="{0}">' + linkText + '</a>' :
    ...
    // valuesIndex 
    // (i.e. valuesIndex = 1 (the index = 0 is the title)
    if ( valuesIndex == null) {
    	valuesIndex = 0; 
	}
        ...
        else
        {
            if(headers)
            {
                ...
                for (i = 0; i < parsedJson.length; i++)
                {
                    ...
                    for (j = valuesIndex; j < headers.length; j++)
                    {
                    	
                        var value = parsedJson[i][headers[j]];
//                    	console.log("parsedJson: " + i + " | " + j + " | " + value );
                        var isUrl = urlRegExp.test(value) || javascriptRegExp.test(value);
                        ...
                        else
                        {
                            if(value){
                            	if(typeof(value) == 'object'){
                            		//for supporting nested tables
                            		tbCon += tdRow.format(ConvertJSONtoSummaryTable(eval(value.data), value.tableId, value.tableClassName, value.linkText));
                            	} else {
                            		tbCon += tdRow.format(value);
                            	}
